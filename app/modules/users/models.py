...
class User(Base):
    ...
    document_sharing = db.relationship('DocumentSharing', lazy='select', backref='shared_user')
    notification = db.relationship('Notification', lazy='select', backref='user')

    # New instance instantiation procedure
    def __init__(self, name, username, hashpass, email=None, role_id=2, library_id=None, 
    ...
    def full_avatar_thumbnail_url(self):
        if self.avatar_thumbnail_url is not None and self.avatar_thumbnail_url != '':
            if os.environ.get('STORAGE_DRIVER') == 'disk':
                return f'{os.environ.get("APP_API_URL")}/files/{self.avatar_thumbnail_url}'
            elif os.environ.get('STORAGE_DRIVER') == 's3':
...
class Role(Base):
    ...
    role_api_route = db.relationship('RoleAPIRoute', lazy="select", backref='role', cascade="all, delete")
    role_web_action = db.relationship('RoleWebAction', lazy="select", backref='role', cascade="all, delete")
    role_mobile_action = db.relationship('RoleMobileAction', lazy="select", backref='role', cascade="all, delete")
