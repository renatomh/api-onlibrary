...
class DocumentCategory(Base):
    ...
    def __init__(self, code, name, library_id=None):
        self.code = code
        self.name = name
...
class Document(Base):
    ...
    document_category_id = db.Column(db.Integer, db.ForeignKey('document_category.id'), nullable=True)
    ...
    file_updated_at = db.Column(db.DateTime, default=db.func.current_timestamp(),
                                        onupdate=db.func.current_timestamp())
    ...
    document_sharing = db.relationship('DocumentSharing', lazy='select', backref='document')
    # model_name = db.relationship('ModelName', lazy='select', backref='document')

    # New instance instantiation procedure
    def __init__(self, code, description, alert, user_id, file_url, file_name, 
        file_content_type, file_size, file_updated_at, file_thumbnail_url=None, 
        file_thumbnail_file_size=None,  observations=None, expires_at=None, 
        alert_email=None, days_to_alert=None, library_id=None, document_category_id=None):
        self.code = code
    ...
    def full_file_thumbnail_url(self):
        if self.file_thumbnail_url is not None and self.file_thumbnail_url != '':
            if STORAGE_DRIVER == 'disk':
                return f'{os.environ.get("APP_API_URL")}/files/{self.file_thumbnail_url}'
            elif STORAGE_DRIVER == 's3':
