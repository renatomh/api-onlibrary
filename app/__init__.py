...
@app.route('/')
def index():
    return redirect(os.environ.get('API_DOC_URL'), 302)
...
def upload_file():
    ...
    if request.method == 'POST':
        ...
        if ('.' in filename and filename.rsplit('.', 1)[1].lower() in app.config['ALLOWED_FILE_EXTENSIONS']):
            ...
            upload_response = store_file(os.path.join(
                app.config['UPLOAD_TEMP_FOLDER'], filename), filename)
        ...
        return jsonify({"data": {},
                        "meta": {"success": False,
                                 "errors": f"File extenstion not allowed {app.config['ALLOWED_FILE_EXTENSIONS']}"}}), 400
...
def not_found(error):
    return jsonify({"data": [],
            "meta": {"success": False,
                     "errors": _("Resource not found.")}}), 404
@app.errorhandler(413)
def ratelimit_handler(e):
    return jsonify({"data": [],
            "meta": {"success": False,
                     "errors": (_('Content too large'), e.description)}}), 413
@app.errorhandler(429)
def ratelimit_handler(e):
    return jsonify({"data": [],
            "meta": {"success": False,
                     "errors": (_('Ratelimit exceeded'), f"{e.description}.")}}), 429
...
app.register_blueprint(mod_country)
# Document modules
